# 多机协同契约 · 平板直连双车 + 多点队列覆盖

**版本 v0.7（2026-06-14）** — 当前主路径定为“平板直连两辆车 UDP + 正式 `routes[]` 多点队列”。平板为每台车分配多个目标点，车端逐点执行普通导航并用 `COOP_AVOID` 做近场让行。旧 LAN 黑板/agent 章节保留为历史兼容和后续可选路径，不再是本轮实机主路径。

## v0.7 正式结论（当前实现依据）

1. 多机覆盖的正式任务字段为 `routes`：
  ...
  "routes": [
    {
      "carId": 0,
      "robotId": 0,
      "points": [{ "x": 120, "y": 80 }, { "x": 130, "y": 95 }],
      "cursor": 0,
      "status": "idle|running|arrived|avoiding|paused|failed|done"
    }
  ]
}
```

2. `points[].x/y` 使用紫派地图坐标系，单位仍为 5cm，与 UDP `cmd3` 目标点一致。
3. 平板是全局规划器和节拍器：等待当前点到达、失败或超时后，再把下一点翻译为普通 UDP `cmd3 -> ROBOT_CONTROL 20`。车端不接收整条点队列，也不把多点列表塞进旧 9 字节 `107/108`。
4. 子车入场顺序固定为：`cmd105` 拉主车 `zipedMap.txt`（失败回退 `defultMap.txt`）→ `cmd5` 加载并归零到 `(0,0,0)` → 按 `routes[carId].points[]` 逐点接收 `cmd3`。
5. 旧 `assignments + cmd107/108 + roadFile.txt` 只作为矩形覆盖兼容路径；新多点队列不依赖 `roadFile.txt`。
6. 协同避障仍在车端 `COOP_AVOID` 内完成，不新增 App 侧避障 UDP 命令；平板通过心跳 byte1/byte2 读取避障状态和事件。
...
| `points[]` | 平板规划出的多点队列，每点 `x/y` 为 5cm 单位。 |
| `cursor` | 平板维护的当前执行点下标。 |
| `status` | 平板/agent 可视化状态；裸 UDP 主路径中由平板结合目标执行结果和心跳 byte1/byte2 维护。 |
...
|---|---|
| `idle` | 尚未开始或无队列。 |
| `running` | 当前点已下发，车端正常导航。 |
| `arrived` | 当前点到达，平板准备下发下一点。 |
| `avoiding` | 心跳 byte1 为 `1` 或 `4`，表示诊断或本地动态避障兜底。 |
| `paused` | 心跳 byte1 为 `2` 或 `3`，表示本车为对车暂停或对车被本车暂停。 |
| `failed` | 当前点超时、无路或外部判定失败。 |
| `done` | 队列全部完成。 |

---

**历史版本 v0.6（2026-06-13）** — App 侧（owner）定方案；多机覆盖主语义从“车端按一个矩形拆分 `roadFile.txt`”调整为“平板为每台车分配不同的多点队列，车端逐点执行并协同避障”。
