...
非不兼容）。**你（紫派）几乎不用改 C/C++**——cmd105/124、cmd5 归零、107/108、心跳、0x06 全部原样复用。
**详见 `contracts/dual-car-purplepi-handoff.md`**，内含：① 保留不动清单 ② 作废的 agent 期请求（心跳回源端口 /
agent 常驻 / DATASYNC / 互信 全不需要）③ 请你真机确认 3 条（主车 :8000 供从车 wget 是否稳/耗时、车↔车 :8000 互通）。
**Q6 / Q6.2 / A6.2「平板不直连任何车」约定就此反转**（方案A 下平板直连每辆车，每车单客户端=平板）。

---

## 🆕 2026-06-13(B) · 多机覆盖：每车「独立矩形·各自全覆盖」是硬需求；覆盖怎么实现请 A 设计并定消息格式 → A

读了 A 的 `plan10.md` + `multi-robot-collab.md` v0.6 + 根 README（`58a6119`/`a7ff9b4`）。先谢 A 把方案A 写进 README、并把 `cmd105` 改成优先拉压缩图 `zipedMap.txt`（子车拉图提速，正合我们 handoff 的诉求）。平板侧就**多机覆盖**定调如下：

**① 硬需求（不变）：每辆车一块独立矩形、各自完整覆盖自己那块。**
- 平板交互 = 操作员给**每辆车各画一个矩形**；任务分配 = 这些矩形本身（均分 / 他多我少都由平板画定）。
- 要的语义是「车1 完整覆盖矩形A、车2 完整覆盖矩形B」，**不是**两车共享一片、master 走前半 / sub 走后半逆序
  （= A 现 `123` 按 `robot_id` 分段的 split 语义）。**那个「共享一片再劈半」模型我们不采用。**
- ⚠️ 据 README「`123`…`robot_id=0` 走前半段、`robot_id=1` 走后半段逆序」：若每辆车各发**自己的**矩形给这套逻辑，
  每辆车只会覆盖自己矩形的**一半** → 对我们是坏的。请按下面 ② 调整。

**② 分工：覆盖路径生成是紫派的事，平板只发消息。**
- 矩形内部怎么扫（牛耕 / BCD / 多点逐点）**请 A 在紫派侧设计**；**平板不做覆盖分解、不算航点**——`plan10` 里
  「平板为每车下发多点队列 `routes[].points[]`」那条**我们不采纳**（平板只表达「这辆车 = 这块矩形」，不替紫派算路）。
- 请 A 设计好后**告诉我们平板该给紫派发什么**：命令码 / 9 字节布局 / 时序。我们 App 按你给的规格发，几乎随你定。
- **最省事的对齐**：沿用现成 `cmd107/108`（每车发**自己的**矩形 + `robot_id` 仅作车号标识）；把 `122/123` 改成
  「**收到某车自己的矩形 → 在该矩形内生成完整覆盖路径并整条执行**」，**去掉前半/后半 split**（split 是「共享一片」的假设）。
  这样 App 一行不用改。若你想换命令/格式也行——给规格即可。

**③ 避让 / 暂停态：你自行设计，我们不强依赖、不阻塞。**
- 你想在心跳留个状态字节标「避让中/暂停」就留，我们最多在地图上加个小图标；**不发也完全可以**——车避让时在我们图上
  就是「停在原地」，我们没有「卡死告警」逻辑、不会误报。按你紫派侧方便来。

**④ 真机「车不动」里与平板相关的只有一项**：若 `COOP_AVOID` 多播在场地网持续 `TIMEOUT`、你最终要走 `plan10` 的
「平板中转协同避让消息」那条，再单独开 spec 给我们（命令/字节/时序），我们配合做中转；**当前不做**。

> 一句话：平板交互与消息我们已就绪（每车一矩形）；**球在紫派**——定下「每车全覆盖自己矩形」的实现 + 对应消息格式，回这条即可。
