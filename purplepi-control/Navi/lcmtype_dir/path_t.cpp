...
int64_t __path_t_hash_recursive(const __lcm_hash_ptr *p)
{
    ...
    int64_t hash = 0xecdb2a054c696658LL
...
int __path_t_encode_array(void *buf, int offset, int maxlen, const path_t *p, int elements)
{
    ...
    for (element = 0; element < elements; element++) {
        ...
        thislen = __int16_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].length), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        { int a;
        for (a = 0; a < p[element].length; a++) {
            thislen = __double_encode_array(buf, offset + pos, maxlen - pos, p[element].xyr[a], 3);
            if (thislen < 0) return thislen; else pos += thislen;
        }
        }
...
int __path_t_decode_array(const void *buf, int offset, int maxlen, path_t *p, int elements)
{
    int pos = 0, thislen, element;
    ...
    for (element = 0; element < elements; element++) {
        ...
        thislen = __int16_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].length), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        p[element].xyr = (double**) lcm_malloc(sizeof(double*) * p[element].length);
        { int a;
        for (a = 0; a < p[element].length; a++) {
            p[element].xyr[a] = (double*) lcm_malloc(sizeof(double) * 3);
            thislen = __double_decode_array(buf, offset + pos, maxlen - pos, p[element].xyr[a], 3);
            if (thislen < 0) return thislen; else pos += thislen;
        }
        }
...
int __path_t_decode_array_cleanup(path_t *p, int elements)
{
    ...
    for (element = 0; element < elements; element++) {
        ...
        for (a = 0; a < p[element].length; a++) {
            ...
            if (p[element].xyr[a]) free(p[element].xyr[a]);
        }
        }
        if (p[element].xyr) free(p[element].xyr);
...
int __path_t_clone_array(const path_t *p, path_t *q, int elements)
{
    ...
    for (element = 0; element < elements; element++) {
        ...
        q[element].xyr = (double**) lcm_malloc(sizeof(double*) * q[element].length);
        { int a;
        for (a = 0; a < p[element].length; a++) {
...
static
void path_t_handler_stub (const lcm_recv_buf_t *rbuf,
...
path_t_subscription_t* path_t_subscribe (lcm_t *lcm,
                    const char *channel,
                    path_t_handler_t f, void *userdata)
{
    path_t_subscription_t *n = (path_t_subscription_t*)
                       malloc(sizeof(path_t_subscription_t));
    n->user_handler = f;
    n->userdata = userdata;
    n->lc_h = lcm_subscribe (lcm, channel,
                                 path_t_handler_stub, n);
    if (n->lc_h == NULL) {
...
int path_t_unsubscribe(lcm_t *lcm, path_t_subscription_t* hid)
{
    ...
    if (0 != status) {
        fprintf(stderr,
           "couldn't unsubscribe path_t_handler %p!\n", hid);
        return -1;
    }
    free (hid);
    return 0;
