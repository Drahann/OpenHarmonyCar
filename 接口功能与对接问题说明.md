...
`Navi/main.cpp` 的地图分片发送逻辑读取首行 4 个值：`range resolution height width`。当前 `contracts/map-format.md` 简化为“行数 列数”不够准确。建议 App 解析兼容首行末两个整数作为高宽，这与 `app-harmony/README.md` 中的折中方案一致。

地图栅格内容以实际文件为准，当前契约里的空格分隔示例仅作示意；App 解析应兼容密排字符或按真实 fixture 修正。
...
- 地图首行建议 App 继续按“取首行最后两个整数作为 height/width”解析，因为紫派分片发送逻辑按 `range resolution height width` 读取。
- 栅格内容以真实文件为准；App 解析应兼容密排字符，也可容忍空格分隔 fixture。
